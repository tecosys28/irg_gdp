...
gcloud run deploy "$SERVICE" \
  ...
  --set-env-vars "DJANGO_ALLOWED_HOSTS=irggdp.com,www.irggdp.com" \
  ...
  --update-secrets "RAZORPAY_KEY_ID=razorpay-key-id:latest" \
  --update-secrets "RAZORPAY_KEY_SECRET=razorpay-key-secret:latest" \
  --update-secrets "EMAIL_HOST_USER=email-host-user:latest" \
  --update-secrets "EMAIL_HOST_PASSWORD=email-host-password:latest"

echo "=== 3. Map custom domain to Cloud Run (one-time) ==="
gcloud beta run domain-mappings create \
  --service "$SERVICE" \
  --domain "irggdp.com" \
  --region "$REGION" \
  --project "$PROJECT_ID" 2>/dev/null || echo "Domain mapping already exists — skipping."

echo "=== 4. Deploy Firebase (hosting + functions + rules) ==="
