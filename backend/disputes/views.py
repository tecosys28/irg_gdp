...
class DisputeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Users see their filed/against disputes, ombudsmen see assigned
        if hasattr(user, 'ombudsman_profile'):
            return Dispute.objects.filter(assigned_ombudsman=user.ombudsman_profile) | \
    ...
    def file(self, request):
        ...
        # Auto-assign ombudsman (round-robin)
        ombudsman = OmbudsmanProfile.objects.order_by('cases_resolved').first()
    ...
    def start_review(self, request, pk=None):
        ...
        if not hasattr(request.user, 'ombudsman_profile'):
            return Response({'error': 'Ombudsman only'}, status=status.HTTP_403_FORBIDDEN)
        
        if dispute.assigned_ombudsman != request.user.ombudsman_profile:
    ...
    def resolve(self, request, pk=None):
        ...
        compensation_amount = request.data.get('compensation_amount', 0)
...
class ResolutionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return Resolution.objects.filter(dispute__filed_by=self.request.user) | \
...
class CompensationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return Compensation.objects.filter(from_party=self.request.user) | \
    ...
    def mark_paid(self, request, pk=None):
        ...
        comp.status = 'PAID'
        comp.payment_tx_hash = blockchain._simulate_tx('COMPENSATION', str(comp.amount))
